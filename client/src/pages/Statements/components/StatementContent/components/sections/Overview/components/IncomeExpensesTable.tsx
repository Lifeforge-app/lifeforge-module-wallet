...
function IncomeExpensesTable({
  ...
  const prevMonthQuery = useQuery(
    forgeAPI.analytics.getCategoriesBreakdown
      .input({ year: prevMonth.year.toString(), month: prevMonth.month.toString() })
      .queryOptions()
  )
  ...
  const currentTotal = useMemo(
    () => Object.values(currentBreakdown).reduce((acc, curr) => acc + curr.amount, 0),
    [currentBreakdown]
  )
      ...
      <table style={{ width: '100%', marginTop: '1.5rem', minWidth: '0' }}>
        <thead>
          <tr style={{ backgroundColor: 'var(--color-custom-500)', color: 'white' }}>
            <th style={{ width: '100%', padding: '0.75rem', textAlign: 'left', fontSize: '1.125rem', fontWeight: '500' }}>Category</th>
            <th style={{ padding: '0.75rem', fontSize: '1.125rem', fontWeight: '500', whiteSpace: 'nowrap' }}>
              {dayjs().year(prevMonth.year).month(prevMonth.month - 1).format('MMM YYYY')}
            </th>
            <th style={{ padding: '0.75rem', fontSize: '1.125rem', fontWeight: '500', whiteSpace: 'nowrap' }}>
              {dayjs().year(year).month(month).format('MMM YYYY')}
            </th>
            <th style={{ padding: '0.75rem', fontSize: '1.125rem', fontWeight: '500', whiteSpace: 'nowrap' }} colSpan={2}>Change</th>
          </tr>
          <tr style={{ backgroundColor: 'var(--color-bg-800)', color: 'white' }}>
            <th style={{ width: '100%', padding: '0.5rem 1rem', textAlign: 'left', fontSize: '1.125rem', fontWeight: '500' }}></th>
            <th style={{ padding: '0.5rem 1rem', fontSize: '1.125rem', fontWeight: '500' }}>RM</th>
            <th style={{ padding: '0.5rem 1rem', fontSize: '1.125rem', fontWeight: '500' }}>RM</th>
            <th style={{ padding: '0.5rem 1rem', fontSize: '1.125rem', fontWeight: '500' }}>RM</th>
            <th style={{ padding: '0.5rem 1rem', fontSize: '1.125rem', fontWeight: '500' }}>%</th>
          </tr>
        </thead>
        <tbody>
              ...
              >
                <td style={{ padding: '0.75rem', fontSize: '1.125rem' }}>
                  <Flex align="center" gap="sm">
                    <Icon icon={category.icon} size="1.5rem" style={{ color: category.color }} />
                ...
                <td style={{ padding: '0.75rem', textAlign: 'right', fontSize: '1.125rem', whiteSpace: 'nowrap', color: isNegativeChange ? '#e11d48' : undefined }}>
                  {change < 0 ? `(${numberToCurrency(Math.abs(change))})` : numberToCurrency(change)}
                </td>
                <td style={{ padding: '0.75rem', textAlign: 'right', fontSize: '1.125rem', whiteSpace: 'nowrap', color: isNegativeChange ? '#e11d48' : undefined }}>
                  {Math.abs(prevAmount) < 0.001 ? '-' : `${((change / prevAmount) * 100).toFixed(2)}%`}
                </td>
              </tr>
            )
          })}
          {(() => {
            ...
            return (
              <tr>
                <td style={{ padding: '0.75rem', fontSize: '1.125rem' }}>
                  <Text size="xl" weight="semibold">Total {type === 'income' ? 'Income' : 'Expenses'}</Text>
                </td>
                ...
                <td style={{ padding: '0.75rem', textAlign: 'right', fontSize: '1.125rem', fontWeight: '500', whiteSpace: 'nowrap', borderTop: '2px solid', borderBottom: '6px double', color: isNegativeChange ? '#e11d48' : undefined }}>
                  {change < 0 ? `(${numberToCurrency(Math.abs(change))})` : numberToCurrency(change)}
                </td>
                <td style={{ padding: '0.75rem', textAlign: 'right', fontSize: '1.125rem', fontWeight: '500', whiteSpace: 'nowrap', borderTop: '2px solid', borderBottom: '6px double', color: isNegativeChange ? '#e11d48' : undefined }}>
                  {Math.abs(prevTotal) < 0.001 ? '-' : `${((change / prevTotal) * 100).toFixed(2)}%`}
                </td>
              </tr>
            )
          })()}
